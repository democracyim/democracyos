...
export default {
  ...
  postcode: (val, option, fn) => {
    val = val.trim()
    fn(!regexps.postcode.test(val) && val !== '' ? t('validators.invalid.postcode') : null)
  },
    
  electoralnumber: (val, option, fn) => {
    val = val.trim()
    fn(!regexps.electoralNumber.test(val) && val !== '' ? t('validators.invalid.electoralnumber') : null)
  },
